      ...
      {games.map((g, gi) => {
        ...
        const modeBadge =
          g.game_mode === "AI Draft" ? (
            ...
            <span className="mode-badge mode-badge-normal">
              ⚔️ Normal Draft
            </span>
          ) : null;
        ...
        return (
          <div className="mh-game" key={gi}>
            ...
            <div className="mh-body">
              <div
                className={`mh-player-col ${firstWon ? "mh-won" : "mh-lost"}`}
              >
                <div
                  className={`mh-player-name ${firstWon ? "winner" : "loser"}`}
                >
                  {firstName}
                  {firstWon ? " 🏆" : ""}
                </div>
...
function CardPool({
  ...
  if (sortMode === "elixir") {
    const sorted = [...filtered].sort((a, b) =>
      elixirAsc ? a.elixir - b.elixir : b.elixir - a.elixir,
    );
    content = (
    ...
    const sorted = [...filtered].sort((a, b) => {
      const tDiff =
        (TIER_ORDER[CARD_TIER[a.name]] ?? 99) -
        (TIER_ORDER[CARD_TIER[b.name]] ?? 99);
      ...
      <div className="card-grid">
        {groupEntries.map(([tier, cards]) => (
          <div key={tier} style={{ display: "contents" }}>
            <div
              className={`type-label tier-label tier-${tier.replace("+", "\\+")}`}
              style={{ gridColumn: "1 / -1" }}
            >
              {TIER_ICONS[tier] || ""}{" "}
              <span style={{ fontSize: ".9rem" }}>{tier}</span>{" "}
    ...
    const sorted = [...filtered].sort((a, b) => {
      const tDiff =
        (TYPE_ORDER[CARD_TYPE[a.name]] ?? 99) -
        (TYPE_ORDER[CARD_TYPE[b.name]] ?? 99);
      if (tDiff !== 0) return tDiff;
      return elixirAsc ? a.elixir - b.elixir : b.elixir - a.elixir;
    });
    const groups = {};
          ...
          const byColor =
            b.by === "random"
              ...
              : b.by === 1
                ...
                : "var(--p2)";
          const byLabel =
            b.by === "random" ? "RANDOM" : b.by === 1 ? p1Name : p2Name;
          return (
            <div className="mini-card" key={i}>
              <img
                src={b.card.iconUrl}
...
function TurnBar({
  ...
  const canUndo = isMultiplayer
    ? draft.whose_turn !== localRole && !nothingToUndo
    : !nothingToUndo;
  ...
  return (
    <div id="turn-bar">
      ...
      <div
        id="timer-block"
        style={{ display: draft.ai_mode ? "none" : "flex" }}
      >
        <button
          className="btn-timer"
          id="btn-undo"
  ...
  return (
    <div className="deck-box">
      ...
      <div className="btn-row" style={{ marginTop: ".75rem" }}>
        <button className="btn btn-copy-green" onClick={onCopyLink}>
          📋 COPY DECK LINK
        </button>
        <a
          className="btn btn-open-cr"
          href={link}
          target="_blank"
          rel="noreferrer"
        >
      ...
      {winnerInfo && (
        <div id="winner-banner" style={{ display: "block" }}>
          ...
          <p id="winner-banner-sub">
            {winnerInfo.winner} defeated {winnerInfo.loser} — result saved to
            output.csv
          </p>
          <button
...
function DoneAiEntry({ entry }) {
  ...
  return (
    <div className="done-ai-entry">
      ...
      {entry.reason && (
        <div className="done-ai-entry-reason">{entry.reason}</div>
      )}
    </div>
  );
}
...
export default function DraftPage() {
  ...
  const localRole =
    parseInt(localStorage.getItem(roleKey(lobbyId)), 10) || null;
  ...
  const isMyTurn =
    draft?.whose_turn === localRole && draft?.status === "active";
  ...
  useEffect(() => {
    ...
    if (
      timerSecs > 0 &&
      timerSecs <= 10 &&
      timerSecs !== lastTickSecRef.current
    ) {
  ...
  function handleCopyLink(player, link) {
    navigator.clipboard.writeText(link).then(() => {
      ...
      setTimeout(
        () => setCopyMsg((prev) => ({ ...prev, [player]: false })),
        2200,
      );
    });
  }
  ...
  const joinUrl = lobbyId
    ? `${window.location.origin}/draft/${lobbyId}/join`
    : "";
      ...
      <main>
        ...
        {showWaiting && (
          ...
          >
            ...
            >
              Share this link with your opponent. The draft will start
                    ...
                    draft.ai_log.map((e, i) => {
                      ...
                      return (
                        <div className="ai-entry" key={i}>
                          ...
                          {e.reason && (
                            <span className="ai-entry-reason">
                              — {e.reason}
                            </span>
                          )}
                        </div>
