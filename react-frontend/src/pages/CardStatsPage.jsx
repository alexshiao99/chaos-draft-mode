      ...
      <main>
        ...
        {!loading && !error && (
          <div className="summary">
            ...
            <div className="stat-pill">
              Cards Shown <span>{filtered.length}</span>
            </div>
          </div>
        )}
        ...
        <div className="controls">
          ...
          <span className="ctrl-label" style={{ marginLeft: '.5rem' }}>
            Tier
          </span>
          <div className="seg">
            {tiers.map((t) => (
        ...
        {!loading && !error && filtered.length === 0 && <div className="no-data">No cards match your filters.</div>}
                  ...
                  return (
                    <tr key={card.name} onClick={() => navigate(`/card/${encodeURIComponent(card.name)}`)}>
                      ...
                      <td
                        style={{
                          color:
                            card.overall_rating == null
                              ? undefined
                              : card.overall_rating >= 0.55
                                ? 'var(--win)'
                                : card.overall_rating <= 0.4
                                  ? 'var(--loss)'
                                  : 'var(--gold)',
                      ...
                      <td className={!card.wins ? 'zero' : ''} style={{ color: card.wins ? 'var(--win)' : undefined }}>
                        {card.wins ?? 0}
                      </td>
                      <td
                        className={!card.losses ? 'zero' : ''}
                        style={{ color: card.losses ? 'var(--loss)' : undefined }}
                      >
                        {card.losses ?? 0}
                      </td>
                      <td
                        className={!card.player_bans ? 'zero' : ''}
                        style={{ color: card.player_bans ? 'var(--ban)' : undefined }}
                      >
                        {card.player_bans ?? 0}
                      </td>
                    </tr>
